...
namespace HawDict.Test
{
    public static class TestUtils
    {
        ...
        {
            LoadAndExecuteTestCases(fileName, () => new T());
        }

        public static void LoadAndExecuteTestCases<T>(string fileName, Func<T> createTest) where T : ITestCase
        {
            var testCases = LoadTestCases<T>(fileName, createTest);
            ExecuteTestCases<T>(testCases);
        }

        public static IReadOnlyDictionary<int, T> LoadTestCases<T>(string fileName, Func<T> createTest) where T : ITestCase
        {
            ...
            Assert.AreNotEqual(0, testCases.Count, "No test cases found!");
        ...
        public static void ExecuteTestCases<T>(IReadOnlyDictionary<int, T> testCases) where T : ITestCase
        {
